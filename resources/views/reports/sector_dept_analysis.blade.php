...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex items-start space-x-4">
                <div class="w-14 h-14 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0">
    ...
    {{-- PDF Export --}}
    <a href="{{ route('reports.export', ['format' => 'pdf', 'type' => $type, 'batch_id' => request('batch_id')]) }}" 
       class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center shadow-sm">
        <svg class="w-4 h-4 mr-2 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
        PDF Export
    </a>

    {{-- Excel Export --}}
    <a href="{{ route('reports.export', ['format' => 'excel', 'type' => $type, 'batch_id' => request('batch_id')]) }}" 
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @foreach($sectors as $sector)
                        ...
                        {{-- SECTOR HEADER ROW --}}
                        <tr class="bg-gradient-to-r from-indigo-50 via-purple-50 to-indigo-50 border-t-2 border-indigo-200">
                        ...
                        @foreach($sector->departments as $dept)
                            @php
                                ...
                                $utilColor = $util > 80 ? 'text-green-600' : ($util > 40 ? 'text-yellow-600' : 'text-red-600');
                            @endphp
                            <tr class="hover:bg-indigo-50/30 transition-all duration-200 group">
                                <td class="px-12 py-4 border-l-4 border-indigo-200 group-hover:border-indigo-400 transition-colors">
                                    <div class="flex items-center">
                                        <div class="w-2 h-2 bg-indigo-400 rounded-full mr-3"></div>
                                        <div>
                                            <span class="font-semibold text-gray-900">{{ $dept->name }}</span>
                                            <span class="ml-2 px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs font-bold rounded">
                                                {{ $dept->abbreviation }}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center justify-center w-10 h-10 bg-gray-100 rounded-lg font-bold text-gray-700">
                                        {{ $dept->sap_dumps_count }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right font-mono font-bold text-gray-900 curr-val" data-raw="{{ $budget }}">
                                    {{ number_format($budget) }}
                                </td>
                                <td class="px-6 py-4 text-right font-mono font-bold text-green-700 curr-val" data-raw="{{ $rel }}">
                                    {{ number_format($rel) }}
                                </td>
                                <td class="px-6 py-4 text-right font-mono font-bold text-blue-700 curr-val" data-raw="{{ $exp }}">
                                    {{ number_format($exp) }}
                                </td>
                                <td class="px-6 py-4 text-center">
                        ...
                        <tr class="bg-gradient-to-r from-indigo-100 to-purple-100 font-bold border-b-2 border-indigo-300">
                            ...
                            <td class="px-6 py-4 text-right font-mono text-indigo-900 curr-val" data-raw="{{ $secBudget }}">
                                {{ number_format($secBudget) }}
                            </td>
                            <td class="px-6 py-4 text-right font-mono text-green-800 curr-val" data-raw="{{ $secRel }}">
                                {{ number_format($secRel) }}
                            </td>
                            <td class="px-6 py-4 text-right font-mono text-blue-800 curr-val" data-raw="{{ $secExp }}">
                                {{ number_format($secExp) }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center justify-center px-3 py-1.5 bg-indigo-200 text-indigo-900 font-black rounded-lg">
                                    {{ round($secUtil, 1) }}%
                                </span>
                            </td>
                        </tr>
                    ...
                    <tr>
                        <td class="px-6 py-5">
                            <div class="flex items-center text-white">
                                <div class="w-10 h-10 bg-white/10 backdrop-blur rounded-xl flex items-center justify-center mr-3">
                        ...
                        <td class="px-6 py-5 text-right font-mono font-black text-base text-white curr-val" data-raw="{{ $gtBudget }}">
                            {{ number_format($gtBudget) }}
                        </td>
                        <td class="px-6 py-5 text-right font-mono font-black text-green-400 text-base curr-val" data-raw="{{ $gtRel }}">
                            {{ number_format($gtRel) }}
                        </td>
                        <td class="px-6 py-5 text-right font-mono font-black text-blue-300 text-base curr-val" data-raw="{{ $gtExp }}">
                            {{ number_format($gtExp) }}
                        </td>
